...
Since the resut is so consistently that mr.ash is worse than lasso here, 
...
Try running longer. Note that now the worse solution has
better objective... unfortunate! One caveat is that, after discussion,
it seems that the computation of the variational objective
may be unreliable for fixed g.
```{r}
...
Interestingly the fitted pi from lasso intialization is almost identical to the
