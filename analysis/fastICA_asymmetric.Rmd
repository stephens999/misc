...
| 1 source ($p=0.1$, $k=20$, $n=200$) | **0.83** | 0.19 | 0.71 | 0.61 | **0.85** | **0.85** |
...
- **Trace Hessian: helps symmetric, hurts sparse**.  For symmetric sources
  ...
  19% vs 6% at $k=20$).  But for sparse sources ($p = 0.1$, $k=20$) it collapses
  from 83% to 19%.  The mechanism is $n/k$ amplification:
  $c_{\text{trace}} = \overline{M'} + (n/k)\,\text{Cov}(M', S)$.
  For symmetric sources $\text{Cov}(M', S) \approx 0$, so trace ≈ fastICA
  but better reflects local geometry; for sparse sources the strong
  negative covariance (active samples: small $M'$, large $S_{ii}$) is
  amplified by $n/k = 10$, shrinking the correction and destabilising Newton steps.
