...
legend("topright",
       c("source (p_true = 0.2)", "Gaussian baseline", "p = 0.2"),
       col = c("steelblue", "grey40", "tomato"),
       lty = c(1, 2, 3), lwd = c(2, 2, 1.5), bty = "n")
```

The asymmetric objective for the $p = 0.2$ source (blue) peaks near
$p_\text{test} = 0.2$ and lies **above** the Gaussian baseline (grey) in that
region — the optimizer can successfully identify the source by maximising over
$p$.  At $p_\text{test} = 0.5$ the asymmetric objective reduces to the
log-cosh contrast, which falls below the Gaussian baseline for this source,
explaining why standard fastICA fails.
...
The asymmetric objective is a maximum (not a minimum), so the **highest** value
should correspond to a true source.  We can check whether this is a reliable
selection criterion:

```{r}
ord9_gr = order(mc9_gr$obj, decreasing = TRUE)   # highest objective first
cols9_gr = ifelse((mc9_gr$maxcor > 0.9)[ord9_gr], "steelblue", "tomato")
plot(seq_along(mc9_gr$obj), mc9_gr$obj[ord9_gr],
     pch = 19, cex = 0.8, col = cols9_gr,
     xlab = "rank (1 = highest asymmetric objective)",
     ylab = "asymmetric log-likelihood at convergence",
     main = "9-groups p=0.2: sorted asymmetric objectives\n(blue = true source found, red = not)")
legend("topright", c("true source (cor > 0.9)", "other"),
       pch = 19, col = c("steelblue", "tomato"), bty = "n", cex = 0.9)
```
...
```{r}
lc_obj_01 = numeric(100)
for (seed in 1:100) {
  set.seed(seed)
  w = rnorm(nrow(Z9_01))
  for (i in 1:200) w = fastica_r1update(Z9_01, w)
  lc_obj_01[seed] = mean(log(cosh(as.vector(t(Z9_01) %*% w))))
}
succ = mc9_01_lc > 0.9
...
sources**.  If the 82 successful runs are concentrated at the bottom of the
sorted objective, one can simply keep the $n_\text{runs}$ with the lowest
log-cosh value:

```{r}
ord = order(lc_obj_01)           # ascending: deepest minimum first
cols = ifelse(succ[ord], "steelblue", "tomato")
plot(seq_along(lc_obj_01), lc_obj_01[ord],
     pch = 19, cex = 0.8, col = cols,
     xlab = "rank (1 = lowest log-cosh objective)",
     ylab = "log-cosh objective at convergence",
     main = "9-groups p=0.1: sorted log-cosh objectives\n(blue = true source found, red = not)")
abline(h = lc_gauss, lty = 2, col = "grey50")
legend("topleft", c("true source (cor > 0.9)", "other", "Gaussian baseline"),
       pch = c(19, 19, NA), lty = c(NA, NA, 2),
       col = c("steelblue", "tomato", "grey50"), bty = "n", cex = 0.9)
```

If the blue points cluster at the left (lowest objective) and red points at the
right, using the objective as a selection criterion recovers true sources
reliably — the "failure rate" of random starts is then irrelevant, since all we
need is that the *best* run (by objective) is a true source.

The asymmetric method maximises its objective, so we check the same for it:

```{r}
succ_gr01 = mc9_01_gr$maxcor > 0.9
ord9_gr01 = order(mc9_01_gr$obj, decreasing = TRUE)
cols9_gr01 = ifelse(succ_gr01[ord9_gr01], "steelblue", "tomato")
plot(seq_along(mc9_01_gr$obj), mc9_01_gr$obj[ord9_gr01],
     pch = 19, cex = 0.8, col = cols9_gr01,
     xlab = "rank (1 = highest asymmetric objective)",
     ylab = "asymmetric log-likelihood at convergence",
     main = "9-groups p=0.1: sorted asymmetric objectives\n(blue = true source found, red = not)")
legend("topright", c("true source (cor > 0.9)", "other"),
       pch = 19, col = c("steelblue", "tomato"), bty = "n", cex = 0.9)
```
