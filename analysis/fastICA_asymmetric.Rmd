...
Remarkably, log-cosh fastICA succeeds 82% of the time at $p \approx 0.1$ —
almost matching the asymmetric method — despite theoretically having
$E[\log\cosh(z)] < $ Gaussian baseline.  This mirrors the result for the
single-source $k = 20$ case and so appears to be a genuine phenomenon.  The
likely explanation is signal strength: at $p = 0.1$ the active samples take
very large values ($y_1 = \sqrt{0.9/0.1} \approx 3$), creating a strong local
gradient toward the true direction even when the global objective maximum is not
there.  At $p = 0.2$ the active-sample values are smaller ($y_1 = 2$) and the
basin of attraction shrinks to essentially zero.  The log-cosh trace Hessian
collapses performance (16%) for the same amplified-covariance reason as before.

The asymmetric method with warm starts achieves 100% — perfectly combining
the good log-cosh initialisation (which works here!) with the correct prior.
...
- **Log-cosh fastICA succeeds at $p = 0.1$ despite theory**.  Standard
  log-cosh has $E[\log\cosh] < $ Gaussian baseline for both $p = 0.2$ and
  $p = 0.1$, yet it achieves 82% at $p \approx 0.1$ (9 groups) and 76% at
  $p = 0.1$ (single source, $k=20$), while failing completely at $p = 0.2$
  in both settings.  The likely mechanism: at $p = 0.1$ the active samples
  reach $y_1 \approx 3$, creating a strong local gradient toward the true
  source even when the global objective maximum is not there.  At $p = 0.2$,
  $y_1 \approx 2$ and the basin collapses.  The asymmetric method does not
  suffer this threshold because it adapts $p$ to match the source.
