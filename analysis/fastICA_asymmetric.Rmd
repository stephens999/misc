...
```{r}
set.seed(3)
n_k20 = 500; p_k20 = 1000; k20 = 20

a_k20 = rnorm(p_k20)

# p = 0.5 (symmetric Rademacher)
...
# p = 0.1 (sparse binary, standardised to zero mean / unit variance)
...
All methods struggle relative to the $k = 9$ case.  With $k = 20$ and only one
source, a random $w$ starts nearly orthogonal to the source direction and must
navigate a 20-dimensional landscape.  Log-cosh is competitive here because at
$p = 0.5$ the asymmetric optimizer tends to drift $\hat p$ away from 0.5
before $w$ has converged (mean $\hat p \approx 0.25$–$0.29$), introducing bias.
Warm-starting from log-cosh largely cures this.
...
The fastICA Hessian outperforms the trace Hessian on random starts for this sparse case.
The gap arises because the trace correction is
$$c_{\text{trace}} = \overline{M'(x)} + \frac{n}{k}\,\mathrm{Cov}(M'(x_i),\, S_{ii})$$
The "on" samples ($p = 0.1$, fraction 10%) have small $M'(x_i) \approx 0$
(posterior saturated) but large $S_{ii}$ (they lie far from the origin along the
source direction), so $\mathrm{Cov}(M', S) < 0$.  With $n/k = 500/20 = 25$
this negative covariance is amplified 25-fold, making $c_{\text{trace}}$
substantially smaller than $\overline{M'(x)}$ and destabilising convergence from
random starts.  Warm-starting from log-cosh recovers most of the gap (91% vs 93%)
by initialising near the true direction, where the covariance term is better behaved.
...
| Single source ($p=0.5$, $k=20$) | **0.49** | 0.35 | 0.21 | 0.52 | 0.49 |
| Single source ($p=0.1$, $k=20$) | 0.84 | **0.94** | 0.71 | 0.93 | 0.91 |
