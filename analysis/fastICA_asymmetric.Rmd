...
Log-cosh fails completely (all max|cor| < 0.9). The asymmetric golden-ratio
anchor nearly perfectly succeeds; the M-estimator anchor is intermediate.
...
| 9 overlapping groups ($p \approx 0.2$, $k=9$) | 0.00 | **0.95** | — |
...
- **The random-start degradation on symmetric sources is an initialization
  artifact**: the $p$-optimizer drifts from 0.5 before $w$ has found a true
  source direction.  Warm-starting from a log-cosh solution (which already
  aligns $w$ with a source) fully recovers the 99% success rate, and the
  estimated $\hat p$ is close to 0.5.
- **Asymmetry recovery** ($k=1$ whitening): $\hat p = \min(p, 1-p)$ correctly
  tracks the true sparse fraction over the range $[0.05, 0.5]$.
- **Practical guidance**: warm-starting from log-cosh is a safe default — it
  matches log-cosh on symmetric sources and retains the asymmetric
  method's advantage when $p$ is learned to be far from 0.5.
