...
$\hat p \approx 0.5$ automatically.
...
- **Tradeoff on symmetric sources**: for symmetric Rademacher sources, the
  asymmetric method's p-optimizer can drift from 0.5 in finite samples,
  changing the objective landscape and reducing the success rate from 99% to
  77%.  Log-cosh remains the preferred choice when sources are known to be
  symmetric.
- **Asymmetry recovery** ($k=1$ whitening): $\hat p = \min(p, 1-p)$ correctly
  tracks the true sparse fraction over the range $[0.05, 0.5]$.
- **Practical guidance**: use asymmetric fastICA (golden anchor) when sources
  are expected to be sparse or skewed ($p \ll 0.5$); use standard log-cosh
