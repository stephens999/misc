...
(objective $\approx 0.280$) because their extremely large active-sample values
($y_1 \approx 3$) lower the log-cosh average maximally.  Failed seeds fall into
shallower spurious minima ($\approx 0.285$).  At $p = 0.2$ ($y_1 = 2$) the true
source minimum is shallower still, and random starts almost never find it.

The log-cosh trace Hessian collapses performance (16%) via the same
amplified-covariance mechanism.  The asymmetric method with warm starts achieves
100%: log-cosh converges to the *correct* (minimum) direction, providing an
excellent starting point for the asymmetric optimizer.
