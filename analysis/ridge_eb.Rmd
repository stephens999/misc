...
$$W:=D^{-1}U'Y \sim N(\mu V'1_p, s^2(s_b^2I+D^{-2})).$$
And we can do inference for $\mu,s,s_b$ under this transformed model.

It is easy to show the mle for $s$ is:
$$\hat{s^2} = (1/k) \sum_j (w_j-\mu z_j)^2/(s^2_b+d_j^{-2})$$
where $k$ is the length of $W$ and $z_j = (V'1_p)_j$.
...
neg_loglik(c(0,0),xx,shat,z)
neg_loglik(c(0.1,0),xx,shat,z)
neg_loglik(c(1,0),xx,shat,z)
neg_loglik(c(log(sb),mu),xx,shat,z)

optim(par=c(0,2),fn=neg_loglik,x=xx,shat=shat,z=z)
