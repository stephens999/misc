...
plot(pip.sim,pip.fm,xlab="PIP (proposed method)",ylab="PIP (FINEMAP)")
points(pip.sim[which(b!=0)],pip.fm[which(b!=0)],col=2,pch=16)
...
33 (for a 4 marker model). Not sure what exactly explains the differences in BFs. (I haven't been careful about 
making sure parameters are same across methods, but generally the BF is somewhat robust to that.)
```{r}
tophits = apply(res.sim$alpha,1,which.max)
markers = colnames(Xresid)[tophits]
log10BF(Xresid[,markers[1:3]],ysim,0.5)
log10BF(Xresid[,markers[1:4]],ysim,0.5)
```
