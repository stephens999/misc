...
Here I'm going to try a different approach, which simply constrains ||X'w + c||=1, which is equivalent to adding a row of 1s to the whitened matrix X.
...
```{r}
fastica_r1update = function(X,w, includeG2=TRUE){
  w= w/sqrt(sum(w^2)) # redundant, but just to be safe
  P = t(X) %*% w
  G = tanh(P)
  G2 = 0
  if(includeG2) # Note: if you don't include G2, this is like the EBCD updates rather than ICA and I believe it will always try to maximize logcosh
    G2 = 1-tanh(P)^2
...
preprocess = function(X, n.comp=10){
  ...
  ## This appears to be equivalent to X1 = (sqrt(n)) * t(svd(X)$v[,1:n.comp])       
  V <- X %*% t(X)/n
  s <- La.svd(V)
  D <- diag(c(1/sqrt(s$d)))
  K <- D %*% t(s$u)
...
Note that whitening the tree effectively results in removing the top shared branch and just leaving an identity matrix:
```{r}
image(t(X1) %*% X1)
```
...
In the above the preprocessing was mostly done with adding minimal noise (ie as if we knew the true number of components). I found that performance of ICA was much degraded, even in simple examples, by more noise. This section was added to document this.

### 9 group simulations

To illustrate, I use the simple 9 group simulation with 20 members each. But in my preprocessing I use 20 components rather than the 10 I used above.
```{r}
K=9
p = 1000
n = 100
...
Here I try with 100 random starts - a minority still find a good group solution. Most of the runs seems to end up at a local minima of the objective rather than a local maximum.
```{r}
obj = rep(0,100)
Lhat = matrix(nrow=100,ncol=n)
for(seed in 1:100){
...
This is the bottom seed: you can see it picks out a "laplace-like" distribution, which gives a small logcosh value.
```{r}
seed= order(obj,decreasing=TRUE)[100]
set.seed(seed)
w = rnorm(nrow(X1))
...
This is the top seed: it picks out the trivial solution.
```{r}
seed= order(obj,decreasing=TRUE)[1]
set.seed(seed)
w = rnorm(nrow(X1))
...
This is the third seed: it picks out a group (second seed also picked out trivial solution)
```{r}
...
plot(t(X1) %*% w)
```
### EBCD, 100 random starts

When we remove the G2 term from the fastICA updates, this is essentially the single-unit
EBCD updates from Kang + Stephens. These are designed to maximize the objective, so I try those here. Now all the runs get a larger value for the objective. However, they mostly don't seem to be quite finding the single group solutions, and indeed further investigation suggests they are finding a combination of groups.
```{r}
obj = rep(0,100)
for(seed in 1:100){
  set.seed(seed)
  w = rnorm(nrow(X1))
  for(i in 1:100)
    w = fastica_r1update(X1,w,includeG2=FALSE)
  obj[seed] = compute_objective(X1,w)
}
plot(obj)
```

This is the bottom seed:
```{r}
seed= order(obj,decreasing=TRUE)[100]
set.seed(seed)
w = rnorm(nrow(X1))
...
This is a middle seed
```{r}
seed= order(obj,decreasing=TRUE)[50]
set.seed(seed)
w = rnorm(nrow(X1))
for(i in 1:100)
  w = fastica_r1update(X1,w,includeG2=FALSE)
...
### EBCD + fastICA updates

It looks like these solutions are "on their way" to binary, but not yet binary. I could
try running more iterations to see if it changes, but here I add on the fastICA updates (with G2) to see what happens. That is, here I'm initializing fastICA with the EBCD results. It does not seem to change much.
```{r}
obj = rep(0,100)
for(seed in 1:100){
  set.seed(seed)
  ...
  for(i in 1:100)
    w = fastica_r1update(X1,w,includeG2=FALSE)
  for(i in 1:100)
    w = fastica_r1update(X1,w,includeG2=TRUE)
  obj[seed] = compute_objective(X1,w)
}
plot(obj)
```
...
Here I try running just a couple of EBCD updates to encourage fastICA to maximize rather than minimize. It definitely helps it to find a local maximum, but a lot of the local optima are still not the single-group solutions.
```{r}
obj = rep(0,100)
for(seed in 1:100){
  set.seed(seed)
...
### EBCD on the original data

Here i run EBCD on the original (unwhitened) data after standardizing. 
However, this just finds the first PC! 
Presumably this is the problem that the fit term is dominating over the prior? 
```{r}
X2 = t(scale(X,center=FALSE)) # make it so the rows are the features and standard devation is 1
set.seed(1)
w = rnorm(nrow(X2))
for(i in 1:100)
  w = fastica_r1update(X2,w,includeG2=FALSE)
cor(L,t(X2) %*% w)
plot(t(X2) %*% w)
plot(svd(X2)$u[,1],w)
```
