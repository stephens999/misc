...
Here we have 4 non-overlapping groups of 25 samples each in n=125, so each source is active
for 20% of samples (p=0.2). We use n=125 rather than n=100 so that the 4 groups do not
perfectly partition all samples — if every sample belongs to exactly one group, the centered
data has rank k-1 rather than k, making the k-th whitened component the constant direction
(which TLC is attracted to). With background samples this rank deficiency does not arise.
...
p=0.2 is near where E[log(cosh(z))] dips furthest below the Gaussian baseline, so this
should be a harder case for standard log-cosh than p=0.5.
